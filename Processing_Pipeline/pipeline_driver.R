...
mwi_pipeline <- function(m_reg_custom = m_reg, custom_data = list(),
    ...
    for (gl in geo_levels) {
      ...
      for (fn in unique(m_reg_sub$Filename)) {
        ...
        if (nrow(comb_df) == 0) {
          ...
          comb_df[
            as.character(curr_df[, geoid_colname]),
  ...
  info_dat[, c(
    "Is_Numeric", "Minimum", "Maximum", "Missing", "Number_Rows",
    ...
    for (gl in geo_levels) {
      ...
      if (nrow(info_dat_sub) > 0) {
        ...
        info_dat[info_dat_sub$Numerator, "Is_Numeric"] <-
          sapply(
            level_data[[gl]][, info_dat_sub$Numerator, drop = F],
            is.numeric
          )
        ...
        for (num in info_dat_sub$Numerator) {
          ...
          for (cn in c(
            "Is_Numeric", "Minimum", "Maximum", "Missing",
            "Number_Rows"
          )) {
            cat(paste0(
              "\t", gsub("_", " ", cn), ":",
  ...
  if (!upd_weights) {
    ...
    meas_df[
      all_pop_df$GEOID[all_pop_df$total_black < 1],
      !grepl("_pop", colnames(meas_df)) &
        colnames(meas_df) %in%
          info_dat$Numerator[info_dat$`Race Stratification`]
    ] <- NA
    meas_df[
      all_pop_df$GEOID[all_pop_df$total_pop < 1],
      grepl("_pop", colnames(meas_df)) |
        colnames(meas_df) %in%
          info_dat$Numerator[!info_dat$`Race Stratification`]
    ] <- NA
