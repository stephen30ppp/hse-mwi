    ...
    names(avail_measures[[idx]]) <-
      c(
        ...
        m_reg[
          gsub(
            "*_pop$", "",
            gsub("*_black$", "", colnames(mwi[[idx]])[-c(1:2)])
          ), "Measure"
        ]
      )
...
zcta_to_zip <- aggregate(ZIP_CODE ~ ZCTA,
  data = zip_cw,
  FUN = function(x) {
    paste(x, collapse = ", ")
  }
...
cty_cw$COUNTY <- aggregate(COUNTY ~ ZCTA5,
  data = county_cw,
  FUN = function(x) {
    paste(x, collapse = "|")
  }
)[, 2]
cty_cw$GEOID <- aggregate(GEOID ~ ZCTA5,
  data = county_cw,
  FUN = function(x) {
    paste(x, collapse = "|")
  }
...
server <- function(input, output, session) {
    ...
    withProgress(
      message = "Uploading custom Mental Wellness Index!",
      {
        if (!is.null(input$custom_data_st)) {
          validate(need(
            endsWith(tolower(input$custom_data_st$datapath), ".rdata"),
  ...
  output$com_report_card_table <- renderDataTable({
    ...
    reportcard <- merge(reportcard, Rank, by = "row.names", sort = FALSE) %>%
      as.data.frame() %>%
      mutate(
        Directionality = ifelse(Directionality == -1, "Obstacle", "Asset"),
          ...
          if (press != "weights") {
            ...
            if (zip_true &
              ...
              (nrow(zip_df) == 1 |
                (nrow(zip_df) > 1 &
                  all(endsWith(
                    tolower(zip_df$name[zip_df$name != "Metadata.xlsx"]),
                    ".csv"
                  ))))) {
