...
# Download Weights
mh_weights <- read_csv(file.path(
  weights_folder,
  "SAMHSA_MHFacility_weights.csv"
))

sa_weights <- read_csv(file.path(
  weights_folder,
...
step_1_fca <- function(data, geo, facil) {
  ...
  for (i in 1:nrow(data)) {
    ...
    data$R[i] <- 1 / sum(filter(
      geo,
      d < data$Distance[i]
    )$POPULATION) * data$weights[i]
  ...
  print(paste0(
    "[", Sys.time(), "] Step 1 of the FCA took ", print(end - start),
    " seconds"
  ))
  return(data)
}
...
step_2_fca <- function(data, geo, facil) {
  ...
  print(paste0(
    "[", Sys.time(), "] Step 2 of the FCA took ", print(end - start),
    " seconds"
  ))
  # Set A variable name based on dataset being used in function
  colnames(geo)[names(geo) == "A"] <- paste0(
    "A",
    "_",
    facil
  )
...
mh_fac <- mh_fac %>%
  select(
    c("name1", "zip", "county.y", "type_facility", "GEOID", "geometry"),
    c(6:174)
  ) %>%
